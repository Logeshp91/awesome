...
const DrawCore = ({
    ...
    onActive: (
      ...
      switch (currentItem.value?.type) {
        case 'polygon':
          ...
          const newPoint = initialItem?.value?.data?.points
          ? initialItem?.value?.data?.points
              ?.split(' ')
              .reduce((arr: any, item: any) => {
                const [x, y] = item.split(',');
                arr.push(
                  `${Number(x) + translationX},${
                    Number(y) + translationY
                  }`,
                );
                return arr;
              }, [])
  ...
  console.log('DONE ITEMS', JSON.stringify(drawState?.doneItems));
