...
import { Path, Ellipse, Rect, Line, G, Polygon} from 'react-native-svg';
...
export default function CurrentAnimatedItem() {
  ...
  const polygonAnimatedProps = useAnimatedProps(
    () => {
      const points =
        currentItem.value?.type === 'polygon'
          ? currentItem.value.data
      ...
      return {
        ...
        fill: 'transparent',
        opacity: currentItem.value?.type === 'polygon' ? 1 : 0,
        strokeWidth:
          currentItem.value?.type === 'polygon'
            ? currentItem.value.strokeWidth
            : 5,
